...
Camera.prototype.okayToMove = function(oldEye, newEye) {
    ...
    // Assuming map is centered at 0,0 in world coords, and each cell is 1x1 unit.
    let mapX = Math.floor(newEye.elements[0] + map_height/2);
    let mapZ = Math.floor(newEye.elements[2] + map_width/2);

    //console.log("mapX:", mapX, "mapZ:", mapZ); // Debugging

    // Check bounds and map value
    if (mapX >= 0 && mapX < map_width && mapZ >= 0 && mapZ < map_height) {
        if (map[mapZ][mapX] > 1) { // Assuming walls are marked with values > 1 (e.g., 2)
            return false; // Collision with wall
        }
    } else {
        return false; // Out of map bounds is also considered not okay to move (optional)
    }
...
/**
* Moves the camera based on pressed keys (WASDQE) and checks for collisions.
*/
Camera.prototype.move = function() {
 ...
 // Movement (WASD)
 if (this.keys['w']) {
     moveVector.add(forward_horizontal); // Use horizontal forward vector
 }
 if (this.keys['s']) {
     moveVector.sub(forward_horizontal); // Use horizontal forward vector
 }
 if (this.keys['a']) {
 ...
 if (this.okayToMove(this.eye, newEye)) { // Check if move is valid
     ...
     // Optionally, add some feedback or adjust moveVector to slide along the wall
     // For now, we simply prevent the movement.
     //console.log("Collision detected, move prevented."); // Debugging
 }
};
